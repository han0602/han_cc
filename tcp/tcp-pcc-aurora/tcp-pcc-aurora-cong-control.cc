...
#include "/home/han/han_cc-master/ns-allinone-3.36.1_backup/ns-3.36.1/scratch/tcp/tcp-pcc-aurora/tcp-socket-base-custom.h"
...
void
RttEstimatorJK::AddNewSample (Time rtt)
{
  if (m_estRtt.IsZero ())
    {
      m_estRtt = rtt;
    ...
    {
      m_estRtt = Time (m_estRtt.GetDouble () * (1.0 - m_alpha) + rtt.GetDouble () * m_alpha); //指数加权移动平均法
...
{
     std::cout << "GetTypeId: " << Simulator::Now().GetSeconds() << std::endl;
...
TcpPccAurora::TcpPccAurora () : TcpCongestionOpsCustom () //用于创建一个 TcpPccAurora 类的对象，并初始化该对象的成员变量。
{
 ...
 // std::cout << "TcpPccAurora: " << Simulator::Now().GetSeconds() << std::endl;
}

TcpPccAurora::TcpPccAurora (const TcpPccAurora &sock) : TcpCongestionOpsCustom (sock) // 用于基于现有的 TcpPccAurora 对象 sock 创建一个新的 TcpPccAurora 对象。这在需要复制一个对象时非常有用。
{
  NS_LOG_FUNCTION (this);
  // std::cout << "Starting sending rate = " << m_sending_rate << std::endl;
  double call_freq = 1.0 / kMonitorIntervalDuration;
  m_rate_controller = new PccCustomRateController (call_freq);
//  std::cout << "TcpPccAurora_para: " << Simulator::Now().GetSeconds() << std::endl;
}
...
void
TcpPccAurora::CongestionStateSet (Ptr<TcpSocketState> tcb,
...
// std::cout << "CongestionStateSet: " << Simulator::Now().GetSeconds() << std::endl;
}
...
TcpPccAurora::UpdatePacingRate (Ptr<TcpSocketState> tcb)
{
 ...
 // std::cout << "UpdatePacingRate: " << Simulator::Now().GetSeconds() << std::endl;
}
...
void
TcpPccAurora::CongControl (Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateConnection &rc,
                           const TcpRateOps::TcpRateSample &rs, int flowId)
{
  NS_LOG_FUNCTION (this << tcb);
  // std::cout<<"this is congcontrol-----------";
  if (m_wait_mode)
    return;


 // 确保 flow_id 对应的 IntervalQueue 存在
    if (m_interval_queues.find(flowId) == m_interval_queues.end()) {
      std::cout<<"------------new interval------------"<<std::endl;
      m_interval_queues[flowId] = new PccMonitorIntervalQueue();  // 插入一个新的 IntervalQueue
  ...
  //修改成了pointer
  m_interval_queues[flowId]->OnPacketAcked(event_time, segmentsAcked, sz, rtt_estimate);
  CheckMonitoringInterval (event_time);
  //  std::cout << "CongControl: " << Simulator::Now().GetSeconds() << std::endl;
...
{
  //  std::cout << "PktsAcked: " << Simulator::Now().GetSeconds() << std::endl;
}
...
TcpPccAurora::OnPacketSent (Ptr<TcpSocketState> tcb, SequenceNumber32 seq, uint32_t sz)
{
  ...
  int flowId = TcpSocketBaseCustom::GetGlobalFlowId();
       // 确保 flow_id 对应的 IntervalQueue 存在
  if (m_interval_queues.find(flowId) == m_interval_queues.end()) {
          m_interval_queues[flowId] = new PccMonitorIntervalQueue();  // 插入一个新的 IntervalQueue
  }
  ...
  if (ShouldCreateNewMonitorInterval (sent_time))
    {
      ...
      m_interval_queues[flowId]->Push (MonitorInterval (m_sending_rate, sent_time + monitor_duration));
      UpdatePacingRate (tcb);
    }
  // m_interval_queue.OnPacketSent (sent_time, seq, sz);
  m_interval_queues[flowId]->OnPacketSent (sent_time, seq, sz);
//  std::cout << "OnPacketSent: " << Simulator::Now().GetSeconds() << std::endl;
}
...
TcpPccAurora::OnPacketLost (SequenceNumber32 seq, uint32_t sz)
{
  ...
  m_interval_queues[flowId]->OnPacketLost (event_time, seq, sz);
  if (m_wait_mode)
    return;
  CheckMonitoringInterval (event_time);
//   std::cout << "OnPacketLost: " << Simulator::Now().GetSeconds() << std::endl;
...
TcpPccAurora::CheckMonitoringInterval (Time event_time)
{
  ...
  while (m_interval_queues[flowId]->HasFinishedInterval (event_time))
    {
      // MonitorInterval mi = m_interval_queue.Pop ();
      MonitorInterval mi = m_interval_queues[flowId]->Pop ();
      mi.flowId = flowId;  // 绑定 flowId
      ...
      m_rate_controller->MonitorIntervalFinished (mi); //PccRateController *ns3::TcpPccAurora::m_rate_controller ,通过 m_rate_controller，TcpPccAurora 类能够调用 PccRateController 的功能
      m_rate_control_lock.unlock ();
    }
   // std::cout << "CheckMonitoringInterval: " << Simulator::Now().GetSeconds() << std::endl;
}

bool
TcpPccAurora::ShouldCreateNewMonitorInterval (Time sent_time)
{
  int flowId = TcpSocketBaseCustom::GetGlobalFlowId();
       // 确保 flow_id 对应的 IntervalQueue 存在
  if (m_interval_queues.find(flowId) == m_interval_queues.end()) {
    ...
    // std::cout << "ShouldCreateNewMonitorInterval: " << Simulator::Now().GetSeconds() << std::endl;
  // return m_interval_queue.Empty () || m_interval_queue.Current ().AllPacketsSent (sent_time);
  return m_interval_queues[flowId]->Empty () || m_interval_queues[flowId]->Current ().AllPacketsSent (sent_time);
}

Time
TcpPccAurora::GetCurrentRttEstimate (Time sent_time)
{
// std::cout << "GetCurrentRttEstimate: " << Simulator::Now().GetSeconds() << std::endl;
  return m_rtt_estimator.GetJkRtt ();
  
}

DataRate
TcpPccAurora::UpdateSendingRate (Time event_time)
{
  m_rate_control_lock.lock ();
  m_sending_rate = m_rate_controller->GetNextSendingRate (m_sending_rate, event_time);
  m_rate_control_lock.unlock ();
  // std::cout << "PCC: rate = " << m_sending_rate << std::endl;
 // std::cout << "UpdateSendingRate: " << Simulator::Now().GetSeconds() << std::endl;
  return m_sending_rate;
  
}

Time
TcpPccAurora::ComputeMonitorDuration (DataRate sending_rate, Time rtt)
{
// std::cout << "ComputeMonitorDuration: " << Simulator::Now().GetSeconds() << std::endl;
  return Time (std::max (kMonitorIntervalDuration * rtt.GetDouble (),
                         kMinimumPacketsPerInterval * kBitsPerByte * kDefaultTCPMSS /
                             sending_rate.GetBitRate () * Time ("1s").GetDouble ()));
