...
void PccCustomRateController::MonitorIntervalFinished(const MonitorInterval& mi) //where we can change state info
{
    ...
    double throughput_1 = 0.0, throughput_2 = 0.0;
    // double throughput = mi_metric_recv_rate(mi);
    // std::cout<<"-------------"<<mi.flowId<<"-------------"<<std::endl;
    if (mi.flowId==0){
        
        double throughput_1 = mi_metric_recv_rate(mi);
    } 
    else{
        // std::cout<<"succccccccccccces"<<mi.flowId<<"-------------"<<std::endl;
        double  throughout_2 = mi_metric_recv_rate(mi);
    }
    double throughput = throughput_1 + throughput_2;

    double latency_s = mi_metric_avg_latency(mi);
    double loss = mi_metric_loss_ratio(mi);

    // grandient reward
    // static double previous_reward = 0.0;
    // double current_reward = (10.0 * throughput / (8 * mi.GetAveragePacketSize())) - 1e3 * latency_s - 2e3 * loss;
    // double relative_reward_change = 0.0;
    // if (current_reward != 0.0) {  // 防止除以零
    //     relative_reward_change = (current_reward - previous_reward) / abs (current_reward);
    // }
    // previous_reward = current_reward;
    // double reward = relative_reward_change;
    ...
    double reward = 0 ;
    double lambda = 1000.0; // 拉格朗日乘子，需根据训练过程动态调整
    // std::cout << "-----------------test_1antency: " << latency_s << std::endl;
   
    // std::cout << "-----------------test_throughput: " << throughput << std::endl;
    // std::cout << "-----------------test_latency: " << latency_s << std::endl;
    // std::cout << "-----------------test_loss: " << loss << std::endl;
    reward = (10.0 * throughput / (8 * mi.GetAveragePacketSize())) - 1e3 * latency_s - 2e3 * loss;
    // std::cout << "-----------------test__reward: " << reward << std::endl;
    if (latency_s > 0.075) {
        reward -= lambda * (latency_s - 0.035); // 添加约束惩罚项
        // std::cout << "-----------------test_punish: " << reward << std::endl;
    }
    ...
    // double min_line = -1000000;
    // double reward = std::max((10.0 * throughput / (8 * mi.GetAveragePacketSize())) - 1e3 * latency_s - 2e3 * loss,min_line);
    // Scaling Parameters
...
bool PccCustomRateController::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
    ...
    m_alpha_sending_rate = box->GetValue(0) * 0.025; // 0.05 is Delta Scale/ original is 0.025 in the article
...
Ptr<OpenGymSpace> PccCustomRateController::GetObservationSpace()
{
 ...
 // --------------------k = 3 ----------------------
    ...
    // std::vector<float> low = {-1., 1., 0., -1., 1., 0., -1., 1., 0.,};
    // std::vector<float> high = {10., 10000.,  1000., 10., 10000.,  1000., 10., 10000.,  1000.,};
...
{
    uint32_t parameterNum = 30; // k = 10
    // uint32_t parameterNum = 9; // k = 3
    std::vector<uint32_t> shape = {parameterNum,};
