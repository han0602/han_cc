...
{ 
  ...
  // double randomValue = uv->GetValue (0.0, 1.0);  // 生成 [0.0, 1.0) 之间的随机数
  // Ptr<Node> currentRouter = m_ipv4->GetObject<Node>();
  // uint32_t currentRouterId = currentRouter->GetId();
  // // std::cout<<"it is the left router"<<currentRouterId<<std::endl;
  // const uint32_t leftRouterId = 0; //0：left-router;1: right-router;2:left sender;4:right-sender
  // if (currentRouterId == leftRouterId)
  // {
  //   // std::cout<<"it is the left router"<<std::endl;
  //  // 如果随机数小于 0.01 (1%)，丢弃该包
  //   if (randomValue < 0.01)
  //     {
  //       NS_LOG_LOGIC ("Randomly dropping packet with probability 1%");
  //       ecb (p, header, Socket::ERROR_NOROUTETOHOST); // 丢弃包并调用错误回调
  //       return false; // 丢包并返回
  //     }
  // }
  // else
  // {
  //   // std::cout<<"not left router"<<std::endl;
  // }
